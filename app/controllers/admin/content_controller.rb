...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    unless current_user.admin?
      flash[:error] = _("Only admins can merge")
      redirect_to :action => 'edit', id: @article.id
      return
    end

    @article = Article.find(params[:id])
    article_id_to_merge = params[:merge_article][:id].to_i

    if @article.id == article_id_to_merge
      flash[:error] = _("Error, merge id must be different")
    else
      if @article.merge_with(article_id_to_merge)
        flash[:notice] = _("Successful merge!")
      else
        flash[:error] = _("Error, problem merging")
      end
    end
